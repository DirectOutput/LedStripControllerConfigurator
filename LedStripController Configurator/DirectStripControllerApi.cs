                ...
                if (FTStatus == FTDI.FT_STATUS.FT_OK && DeviceCnt > 0)
                {
                    ...
                    if (FTStatus == FTDI.FT_STATUS.FT_OK)
                    {
                        ...
                        foreach (FTDI.FT_DEVICE_INFO_NODE DI in Devices)
                        {
                            ...
                            {
                                if (DI.Description == Properties.Settings.Default.LedStripControllerDeviceDescriptionBase.Trim() + " " + ControllerNumber)
                                {
            ...
            if (FTStatus == FTDI.FT_STATUS.FT_OK && DeviceCnt > 0)
            {
                ...
                if (FTStatus == FTDI.FT_STATUS.FT_OK)
                {
                    foreach (FTDI.FT_DEVICE_INFO_NODE DI in Devices)
                    {
                        ...
                        {
                            int ControllerNr = 0;
                            if (DI.Description.StartsWith(Properties.Settings.Default.LedStripControllerDeviceDescriptionBase.Trim()) && int.TryParse(DI.Description.Substring(Properties.Settings.Default.LedStripControllerDeviceDescriptionBase.Trim().Length), out ControllerNr))
                            {
                                if (ControllerNr > 0)
                                {
                                    L.Add(ControllerNr);
